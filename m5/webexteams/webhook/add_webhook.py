...
def main():
    ...
    get_resp = requests.get(
        f"{api_path}/people?email=globobot@webex.bot", headers=headers
    )
    ...
    post_resp = requests.post(
        f"{api_path}/webhooks", headers=headers, json=webhook_data
    )
    ...
    print(f"Webhook for {webhook_data['targetUrl']} added\n with ID {webhook_id}")
